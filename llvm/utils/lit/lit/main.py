...
class TestingProgressDisplay:
    ...
    def handleUpdate(self, test):
        ...
        elif self.opts.showAllOutput:
            print "%s TEST '%s' %s %s" % ('*'*20, test.getFullName(),
                "NOT PASSED" if test.result.isFailure else "PASSED", '*'*20)
            print test.output
            print "*" * 20
...
def getTestsInSuite(ts, path_in_suite, litConfig,
    ...
    if litConfig.excludeUnsupported and lc.unsupported:
        return
...
xmlIllegalCharsRe = re.compile('[\x00-\x08\x0b\x0c\x0e\x0f]')

def genXmlReport(tests, outfile, showAllOutput):
    from xml.sax.saxutils import escape, quoteattr

    def escapeOutput(output):
        return escape(xmlIllegalCharsRe.subn('?', output)[0].encode('UTF-8'))

    suites = {}
    gstats = {'a': len(tests), 'f': 0, 'e': 0, 'd': 0, 't': 0.0}
    for t in tests:
        suites.setdefault(t.suite, ([], {'f': 0, 'e': 0, 'd': 0, 't': 0.0}))
        suites[t.suite][0].append(t)

        gstats['t'] += t.elapsed
        suites[t.suite][1]['t'] += t.elapsed

        if t.result in [Test.PASS, Test.XFAIL]:
            pass
        elif t.result in [Test.FAIL, Test.XPASS]:
            gstats['f'] += 1
            suites[t.suite][1]['f'] += 1
        elif t.result == Test.UNSUPPORTED:
            gstats['d'] += 1
            suites[t.suite][1]['d'] += 1
        else:
            gstats['e'] += 1
            suites[t.suite][1]['e'] += 1

    suites = suites.items()
    suites.sort(key = lambda (ts,_): ts.name)

    outfile.write('<?xml version="1.0" encoding="UTF-8"?>\n')
    outfile.write(('<testsuites tests="%(a)d" failures="%(f)d" ' +
                   'disabled="%(d)d" errors="%(e)d" time="%(t)d" ' +
                   'name="lit-tests">\n') % gstats)

    for (suite, (stests, sstats)) in suites:
        sstats['a'] = len(stests)
        sstats['n'] = quoteattr(suite.name)

        outfile.write(('  <testsuite name=%(n)s tests="%(a)d" ' +
                       'failures="%(f)d" disabled="%(d)d" ' +
                       'errors="%(e)d" time="%(t)f">\n') % sstats)

        for t in stests:
            outfile.write('    <testcase name=%s time="%f"' % (
                    quoteattr('/'.join(t.path_in_suite)), t.elapsed))

            if t.result in [Test.PASS, Test.XFAIL] and not showAllOutput:
                outfile.write(' />\n')

            else:
                outfile.write('>\n')
                if t.result in [Test.PASS, Test.XFAIL]:
                    outfile.write('      <system-out>')
                    outfile.write(escapeOutput(t.output))
                    outfile.write('</system-out>\n')
                elif t.result in [Test.FAIL, Test.XPASS]:
                    outfile.write('      <failure message=%s />\n' % \
                                            (quoteattr(t.result.name),))
                    outfile.write('      <system-out>')
                    outfile.write(escapeOutput(t.output))
                    outfile.write('</system-out>\n')
                elif t.result == Test.UNSUPPORTED:
...
def main(builtinParameters = {}):    # Bump the GIL check interval, its more important to get any one thread to a
    ...
    group.add_option("-v", "--verbose", dest="showOutput",
                     help="Show failed test output",
                     action="store_true", default=False)
    group.add_option("-w", "--very-verbose", dest="showAllOutput",
                     help="Show all test output",
                     action="store_true", default=False)
    group.add_option("", "--no-progress-bar", dest="useProgressBar",
                     help="Do not use curses based progress bar",
                     action="store_false", default=True)
    group.add_option("", "--xml-report", dest="xmlReport", metavar="FILE",
                     help="Generate XML report of test results",
                     action="store", default=None)
    ...
    group.add_option("", "--skip-tests", dest="skipTests", metavar="N",
                     help="Skip the first N tests (to be used with --max-tests)",
                     action="store", type=int, default=None)
    group.add_option("", "--max-tests", dest="maxTests", metavar="N",
                     help="Maximum number of tests to run",
                     action="store", type=int, default=None)
    group.add_option("", "--slice-tests", dest="sliceTests", metavar="N/M",
                     help="Test N-th out of M slices of tests",
    ...
    group.add_option("", "--filter", dest="filter", metavar="EXPRESSION",
                     help=("Only run tests with paths matching the given "
                           "regular expression"),
                     action="store", default=None)
    group.add_option("", "--exclude-unsupported", dest="excludeUnsupported",
                     help="Exclude unsupported tests",
                     action="store_true", default=False)
    parser.add_option_group(group)
...
# threads by default there.
    ...
    litConfig = LitConfig.LitConfig(progname = os.path.basename(sys.argv[0]),
                                    path = opts.path,
                                    quiet = opts.quiet,
                                    showAllOutput = opts.showAllOutput,
                                    ...
                                    isWindows = (platform.system()=='Windows'),
                                    params = userParams,
                                    excludeUnsupported = opts.excludeUnsupported)
    ...
    if opts.skipTests is not None:
        tests = tests[opts.skipTests:]

    if opts.maxTests is not None:
        tests = tests[:opts.maxTests]

    if opts.sliceTests is not None:
        try:
            n, f = [int(x) for x in opts.sliceTests.split(':')]
        except ValueError:
            parser.error("invalid argument for --slice-tests: %s" %(
                    opts.sliceTests,))
        if f <= 0 or n <= 0 or n > f:
            parser.error("invalid argument for --slice-tests: %s" %(
                    opts.sliceTests,))
        l = int(math.ceil(float(len(tests))/f))
        tests = tests[(n-1)*l:n*l]
    ...
    if opts.xmlReport is not None:
        outfile = file(opts.xmlReport, 'w')
        genXmlReport(tests, outfile, showAllOutput=litConfig.showAllOutput)
