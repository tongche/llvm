...
CheckFilenames(cl::Positional, cl::desc("<check-files>"), cl::OneOrMore);
...
IgnoreOrder("ignore-order",
            cl::desc("Ignore order of the strings in the file"));
  ...
  while (!PatternStr.empty()) {
    ...
    if (PatternStr.startswith("[[")) {
      ...
      if (NameEnd == StringRef::npos) {
        if (Name == "LINE_NO")
          RegExStr += Twine(SM.FindLineNumber(PatternLoc)).str();
        else if (Name == "NEXT_LINE_NO")
          RegExStr += Twine(SM.FindLineNumber(PatternLoc)+1).str();
        else
...
static bool ReadCheckFile(SourceMgr &SM, StringRef CheckFilename,
                          std::vector<CheckString> &CheckStrings,
                          std::vector<std::pair<SMLoc, Pattern> > &NotMatches) {
  // Open the check file, and tell SourceMgr about it.
  OwningPtr<MemoryBuffer> File;
  if (error_code ec =
        MemoryBuffer::getFileOrSTDIN(CheckFilename.str().c_str(), File)) {
  ...
  while (1) {
    ...
    // CHECK-NEXT can not be used in IgnoreOrder mode.
    if (IsCheckNext) {
        if (IgnoreOrder) {
          SM.PrintMessage(SMLoc::getFromPointer(CheckPrefixStart),
                          SourceMgr::DK_Error,
                          "found '"+CheckPrefix+"-NEXT:' in ignore-order mode");
          return true;
        } else if (CheckStrings.empty()) {
          SM.PrintMessage(SMLoc::getFromPointer(CheckPrefixStart),
                          SourceMgr::DK_Error,
...
/// ReadCheckFiles - Read all check files.
static bool ReadCheckFiles(SourceMgr &SM,
                           std::vector<CheckString> &CheckStrings) {
  std::vector<std::pair<SMLoc, Pattern> > NotMatches;

  for (unsigned i = 0, e = CheckFilenames.size(); i != e; ++i) {
    if (ReadCheckFile(SM, CheckFilenames[i], CheckStrings, NotMatches))
      return true;
  }

  // Add an EOF pattern for any trailing CHECK-NOTs.
  if (!NotMatches.empty()) {
    const char* E = SM.getMemoryBuffer(CheckFilenames.size()-1)->getBufferEnd();
    CheckStrings.push_back(CheckString(Pattern(true),
                                       SMLoc::getFromPointer(E),
                                       false));
    std::swap(NotMatches, CheckStrings.back().NotStrings);
  }
...
int main(int argc, char **argv) {
  ...
  std::vector<CheckString> CheckStrings;
  if (ReadCheckFiles(SM, CheckStrings))
    return 2;
  ...
  if (IgnoreOrder) {
    // Check that we have all of the expected strings, in any order, but
    // without overlaps. We copy the input buffer, so that we could clobber
    // the copy to mark overlaps, yet use the original for error reporting.
    StringRef SrcBuffer = F->getBuffer();
    MemoryBuffer *FP = MemoryBuffer::getMemBufferCopy(SrcBuffer);
    StringRef Buffer = FP->getBuffer();
    for (unsigned StrNo = 0, e = CheckStrings.size(); StrNo != e; ++StrNo) {
      const CheckString &CheckStr = CheckStrings[StrNo];
      assert(CheckStr.NotStrings.empty() || StrNo+1 == e);
      ...
      if (MatchPos == StringRef::npos) {
        PrintCheckFailed(SM, CheckStr, SrcBuffer, VariableTable);
        return 1;
      }

      while (MatchPos != StringRef::npos && MatchLen != 0) {
        // Override the content to disallow overlaps. We use '\n' as the
        // regex '.' will not match it
        std::memset(const_cast<char*>(Buffer.data())+MatchPos, '\n', MatchLen);
        MatchPos = CheckStr.Pat.Match(Buffer, MatchLen, VariableTable);
      }
    }

    // If there are "not strings", verify that they don't exist in the input.
    const CheckString &LastStr = CheckStrings.back();
    for (unsigned ChunkNo = 0, e = LastStr.NotStrings.size();
         ChunkNo != e; ++ChunkNo) {
      size_t MatchLen = 0;
      size_t Pos = LastStr.NotStrings[ChunkNo].second.Match(Buffer,
                                                            MatchLen,
                                                            VariableTable);
      if (Pos == StringRef::npos) continue;

      SM.PrintMessage(SMLoc::getFromPointer(SrcBuffer.data()+Pos),
                      SourceMgr::DK_Error,
                      CheckPrefix+"-NOT: string occurred!");
      SM.PrintMessage(LastStr.NotStrings[ChunkNo].first, SourceMgr::DK_Note,
  ...
  for (unsigned StrNo = 0, e = CheckStrings.size(); StrNo != e; ++StrNo) {
    ...
    StringRef SkippedRegion = StringRef(LastMatch, Buffer.data()-LastMatch);
